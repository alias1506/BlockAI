...
import com.blockai.movement.PathNode;
...
public class DependencyResolver {
        ...
        if (success) {
            ...
            for (String nodeId : resolutionOrder) {
                ...
                if (node != null && node.getAction() != null) {
                    ...
                    plan.add(subTask);
                } else if (nodeId.startsWith("ITEM:") && ResourceCategory.fromName(nodeId.replace("ITEM:", "")) != null) {
                    // Handle dynamically added abstract category nodes
                    String catName = nodeId.replace("ITEM:", "");
                    RoadmapTask subTask = new RoadmapTask(-1, "gather", "Gather " + catName);
                    subTask.setResource(catName, null, 1);
                    plan.add(subTask);
                }
            }
        } else {
    ...
    private static boolean resolveNode(String nodeId, KnowledgeGraph graph, AIPlayer player, List<String> visited, List<String> order) {
        ...
        if (node == null) {
            if (nodeId.startsWith("ITEM:")) {
                String catName = nodeId.replace("ITEM:", "");
                if (ResourceCategory.fromName(catName) != null) {
                    // It's a valid semantic category, treat it as a dynamic node
                    if (isSatisfiedCategory(catName, player)) {
                        System.out.println("[BlockAI] DependencyResolver: Abstract category " + nodeId + " is already satisfied. Pruning.");
                        return true;
                    } else {
                        visited.add(nodeId);
                        order.add(nodeId);
            ...
            System.out.println("[BlockAI] DependencyResolver Warning: PathNode not found in graph: " + nodeId);
            return false; 
        }
        
        // Check if condition is already satisfied (Pruning)
        if (isSatisfied(node, player)) {
            System.out.println("[BlockAI] DependencyResolver: PathNode " + nodeId + " is already satisfied. Pruning branch.");
            return true;
        }
    ...
    private static boolean isSatisfiedCategory(String catName, AIPlayer player) {
        ResourceCategory cat = ResourceCategory.fromName(catName);
