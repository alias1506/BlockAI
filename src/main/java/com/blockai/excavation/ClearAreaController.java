...
public class ClearAreaController {
    ...
    public TaskResult tick(AIPlayer player) {
        ...
        MiningController.MiningState actionState = miningAction.getState();
        if (actionState != MiningController.MiningState.IDLE) {
            MiningController.MiningState result = miningAction.tick(player);
            if (result == MiningController.MiningState.SUCCESS) {
                System.out.println("[BlockAI] ClearAreaController: Obstacle removed at " + currentTarget);
                completedTargets.add(currentTarget);
                WorldScanner.MEMORY.removeBlockByPos(currentTarget);
                currentTarget = null;
                miningAction.reset();
                tickDelay = 5; // Brief pause before next target
            } else if (result == MiningController.MiningState.FAILED) {
        ...
        if (currentTarget != null && player.movementController.hasPath()) {
            com.blockai.movement.MovementController.MovementState movState = player.movementController.getState();
            if (movState == com.blockai.movement.MovementController.MovementState.FAILED) {
                System.out.println("[BlockAI] ClearAreaController: Target rejected — path failed during movement to " + currentTarget);
                failedTargets.add(currentTarget);
                currentTarget = null;
        ...
        if (currentTarget != null && !player.movementController.hasPath()) {
            BlockScanner.ValidationResult validation = BlockScanner.validate(player, currentTarget);
            if (validation.status == BlockScanner.ValidationStatus.VALID) {
                miningAction.startMining(player, currentTarget);
                return TaskResult.RUNNING;
            } else if (validation.status == BlockScanner.ValidationStatus.TOO_FAR) {
                // Need to get closer — find interaction position
                BlockPos interactPos = BlockScanner.findInteractionPosition(level, player, currentTarget);
                if (interactPos != null) {
                    var path = com.blockai.movement.PathFinder.findPath(level, player.blockPosition(), interactPos);
                    if (path != null) {
                ...
                System.out.println("[BlockAI] ClearAreaController: Target rejected — " + validation.status + " at " + currentTarget);
                failedTargets.add(currentTarget);
                currentTarget = null;
                tickDelay = 5;
                return TaskResult.RUNNING;
