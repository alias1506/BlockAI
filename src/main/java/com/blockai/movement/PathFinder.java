...
public class PathFinder {
    ...
    public static List<BlockPos> findPath(ServerLevel level, BlockPos start, BlockPos target) {
        ...
        PathNode startNode = new PathNode(start, null, 0, getDistance(start, target));
        ...
        while (!openSet.isEmpty() && nodesEvaluated < MAX_NODES) {
            ...
            for (BlockPos neighborPos : getNeighbors(level, current.pos)) {
                ...
                if (neighborNode == null) {
                    neighborNode = new PathNode(neighborPos, current, tentativeGCost, getDistance(neighborPos, target));
    ...
    private static List<BlockPos> retracePath(PathNode endNode) {
